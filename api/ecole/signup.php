...
if (
    ...
    $query = mysqli_query($con, "select * from ecole where email='$email' or nom='$nom'");
    $count = 0;
    while ($row = mysqli_fetch_array($query)) $count++;

    if ($count != 0) {
        echo "<script>alert('Un utilisateur avec ces informations est déjà enregistré !');</script>";
        echo "<script type='text/javascript'> document.location ='http://tabiblio.loc:81'; </script>";
        exit;
    }
    ...
    $path_to_file = "/uploads/ecole/" . $file_name;
    ...
    if (isset($_POST["submit"])) {
        ...
        if ($check !== false) {
            ...
            $uploadOk = 0;
            echo "<script>alert('Le fichier de la logo n\'est pas une image !');</script>";
            echo "<script type='text/javascript'> document.location ='http://tabiblio.loc:81'; </script>";
            exit;
        }
    }
    ...
    if ($_FILES["logo"]["size"] > 500000) {
        $uploadOk = 0;
        echo "<script>alert('Le fichier de la logo est trop grand ! Taille max : 500kb.');</script>";
        echo "<script type='text/javascript'> document.location ='http://tabiblio.loc:81'; </script>";
        exit;
    }
    ...
    if (
        ...
        $uploadOk = 0;
        echo "<script>alert('Le fichier de la logo n\'est pas valide ! Types autorisés : JPG, JPEG, PNG & GIF.');</script>";
        echo "<script type='text/javascript'> document.location ='http://tabiblio.loc:81'; </script>";
        exit;
    }
    ...
    if ($uploadOk == 0) {
        echo "<script>alert('Le fichier de la logo n\'a pas pu être téléversé !');</script>";
        echo "<script type='text/javascript'> document.location ='http://tabiblio.loc:81'; </script>";
        exit;
        ...
        if (move_uploaded_file($_FILES["logo"]["tmp_name"], $target_file)) {
            ...
            $query = mysqli_query($con, "insert into ecole(nom, libelle, mdp, email, logo) value( '$nom','$libelle', '$mdp', '$email', '$target_file' )");
            ...
            echo "<script>alert('Une erreur s\'est produite lors du téléversement de la logo.');</script>";
            echo "<script type='text/javascript'> document.location ='http://tabiblio.loc:81'; </script>";
            exit;
        }
    }
...
<body style="background-image:url(http://tabiblio.loc:81/Assets/triangles-1430105_1280.png); background-size:cover;background-attachment:fixed;background-position: center;">
    ...
    <section style="display:grid;justify-content:center;align-items: center;color:white;height:50vh;">
        ...
        <p>Votre école est la bienvenue sur Tabiblio. Connectez-vous à votre compte pour commencer.</p>
